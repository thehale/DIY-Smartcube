...
class AbsolutePositioning:
    ...
    def __init__(self):
        ...
        for line in lines:
            parts = line.split(" ")
            self.positions[parts[0]] = float(parts[1])

        self.state = {'U': 3, "D": 3, "F": 3, "B": 3, "R": 3, "L": 3}
        for k, v in self.positions.items():  # Included to double check proper reading of the file.
            print("%s %s" % (k, v))
    ...
    def appendStateToSound(self, tps: float):
        self.mixer.add_tone('U', self.positions['U' + str(self.state['U'])], 1/tps)
        self.mixer.add_tone('D', self.positions['D' + str(self.state['D'])], 1/tps)
        self.mixer.add_tone('F', self.positions['F' + str(self.state['F'])], 1/tps)
        self.mixer.add_tone('B', self.positions['B' + str(self.state['B'])], 1/tps)
        self.mixer.add_tone('R', self.positions['R' + str(self.state['R'])], 1/tps)
        self.mixer.add_tone('L', self.positions['L' + str(self.state['L'])], 1/tps)
